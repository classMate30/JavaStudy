...
※  오토 박싱(Auto-Boxing)과 오토 언박싱(Auto-Unboxing)
	...
	이 경우 발생하는 문제 해결을 위해 자바에서는 Wrapper클래스를 제공하게 되었다
	하지만 JDK 1.5부터 레퍼런스 형과 기본자료형이
	다음과 같이 형 변환이 가능하게 되었다

	자유롭게 돌아다니는 primitive를 딱딱한 객체에 가두니까 박싱
	int a=10, b;
	Integer ob;
	ob = a;
	...
	딱딱한 객체에 갖혀있는애를 프리미티브로 자유롭게 풀어주니 언박싱
	b = ob;
	//-- Integer(객체)가 기본 자료형 int형으로 자동 변환(오토 언박싱)
	//	 실제로는 ... 『b = ob.intValue();』


	이는 JDK 1.5이후 추가된 오토 박싱 / 오토 언박싱이라는 
	기능으로 인해 가능해 진 것이다
...
public class Test130
{
	...
	{	
		...
		System.out.println("d.toString() : " + d.toString());
		//--==>>d.toString() : 10	★★★오버라이딩 된 것임 그래서 결과가 다름★★★
