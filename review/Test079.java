...
○ 배열의 개요
	...
	배열의 키포인트
	①동질적인 성격을 가진 자료들
	②자바에선 길이 변환이 불가능하다(가변배열은 사실 메모리에 공간을 다시 확보하는 과정을 거친다.)


○ 형식 및 구조
   
   자료형[] 변수명;
   변수명 = new 자료형[요소크기];  // [] 안을 보통 배열의 길이라고 표현함
   변수명[인덱스] = 값;
...
public class Test079
{
	...
	{
		// 배열 선언
		int[] arr;	//이름과 유형만 잡아 둔 것

		// 메모리 할당
		arr = new int[10]; //10개짜리 집을 만들건데 주소는 arr로 접근할거야!

		// 배열은 다음과 같이 선언과 동시에 메모리 할당 및 초기화 가능하다
		int[] arr2 = new int[10];				// 42번과 45번을 결합시켜 놓은 형태임
		int[] arr3 = {1,2,3,4,5,6,7,8,9,0};		// {}안의 요소 개수만큼 방이 만들어짐
		int arr5[] = {1,2,3,4,5,6,7,8,9,0};		// 일반적으론 49번처럼 쓰는데 50번처럼도 사용 가능
		...
		// ※메모리의 주소값이라고 표현되는 책이 있는데 틀린 것임
		//	 출력되는 값은 메모리의 주소값이 아니다.
		//	 (자바는 메모리 주소값에 사용자가 직접 접근하는 것을 허용하지 않기 때문에)
		//	★-----------------------------------------------------------------------★
		//	 일단은 시리얼 넘버라고 이해할 것. 자바는 메모리 주소값에 사용자가 직접 접근하는 것을 금하는것이 핵심
